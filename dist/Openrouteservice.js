...
export default class Openrouteservice {
    ...
    async getGeocodeSearch(query, additionalQuery) {
        ...
        return this.orsFetch('/geocode/search' + (structured ? '/structured' : ''), false, 
        //@ts-ignore
        new URLSearchParams(params).toString()).then(Openrouteservice.unflattenResult);
    }
    async getGeocodeAutocomplete(text, query) {
        ...
        return this.orsFetch('/geocode/autocomplete', false, 
        //@ts-ignore
        new URLSearchParams(params).toString()).then(Openrouteservice.unflattenResult);
    }
    async getGeocodeReverse(query) {
        ...
        if (Array.isArray(query.layers))
            ...
            query.layers = query.layers.join(',');
        const params = flatten(query);
        return this.orsFetch('/geocode/reverse', false, 
        //@ts-ignore
        new URLSearchParams(params).toString()).then(Openrouteservice.unflattenResult);
    }
    async getPOIs(query) {
